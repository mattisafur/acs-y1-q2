...
void time_of_flight_handler(void *) {}
...
esp_err_t time_of_flight_init(void)
{
    ...
    ESP_LOGI(TAG, "VL53L1X Sensor ID: 0x%04X", id);
    ...
    BaseType_t task_created = xTaskCreate(time_of_flight_handler, "Time_of_Flight_task", 4096, NULL, tskIDLE_PRIORITY + 1, NULL);
...
static void time_of_flight_handler(void *arg)
{
                ...
                if (r.status == 0)
                {
                    if (r.distance_mm > 200)
                    {
                        const orchastrator_return_message_t tof_message = {
                            ...
                            .event = TIME_OF_FLIGHT_EVENT_TRIGGERED,
                        };
